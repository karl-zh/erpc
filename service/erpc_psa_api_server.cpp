/*
 * Generated by erpcgen 1.7.1 on Thu May  9 17:09:47 2019.
 *
...
//! @brief Function to read struct psa_invec
static void read_psa_invec_struct(erpc::Codec * codec, psa_invec * data);

//! @brief Function to read struct psa_outvec
static void read_psa_outvec_struct(erpc::Codec * codec, psa_outvec * data);


// Read struct psa_invec function implementation
static void read_psa_invec_struct(erpc::Codec * codec, psa_invec * data)
{
    codec->read(&data->base);

    codec->read(&data->len);
}

// Read struct psa_outvec function implementation
static void read_psa_outvec_struct(erpc::Codec * codec, psa_outvec * data)
{
    codec->read(&data->base);
...
erpc_status_t PsaCallService_service::psa_call_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    ...
    psa_handle_t handle;
    psa_invec *in_vec = NULL;
    in_vec = (psa_invec *) erpc_malloc(sizeof(psa_invec));
    if (in_vec == NULL)
    {
        codec->updateStatus(kErpcStatus_MemoryError);
    }
    uint32_t in_len;
    psa_outvec *out_vec = NULL;
    out_vec = (psa_outvec *) erpc_malloc(sizeof(psa_outvec));
    if (out_vec == NULL)
    {
        codec->updateStatus(kErpcStatus_MemoryError);
    }
    ...
    read_psa_invec_struct(codec, in_vec);

    codec->read(&in_len);

    read_psa_outvec_struct(codec, out_vec);
